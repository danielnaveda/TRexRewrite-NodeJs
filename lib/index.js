...
app.get('/events/:connID', function(req, res) {
    // res.json(JSON.parse(trex.get_notification(parseInt(req.params.connID))));
    // console.log('connID:', trex.get_notification(req.params.connID));

    // res.json(JSON.parse(trex.get_notification(req.params.connID)));
...
app.post('/define-rule', function(req, res) {
    res.json(JSON.parse(trex.declare_rule(JSON.stringify(req.body))));
});
